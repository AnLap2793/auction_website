        ...
        return {
            ...
            data: {
                ...
                registrations: registrations.map((reg) => ({
                    ...
                    status: reg.status,
                    deposit_status: reg.deposit_status,
...
const updateAuctionStatus = async () => {
    try {
        ...
        for (const auction of activeAuctions) {
            ...
            const winner = auction.current_winner_id ? await User.findByPk(auction.current_winner_id) : null;
            ...
            if (winner) {
                ...
                // Tìm transaction deposit đã thanh toán của user cho auction này
                ...
                // Tính số tiền cần thanh toán (trừ đi tiền đặt cọc đã thanh toán)
                let finalAmount = parseFloat(auction.current_bid);
                let depositInfo = null;

                if (depositTransaction) {
                    const depositAmount = parseFloat(depositTransaction.amount);
                    finalAmount = parseFloat(auction.current_bid) - depositAmount;
                    ...
                    depositInfo = {
                        ...
                        original_amount: parseFloat(auction.current_bid),
                        final_amount: finalAmount,
                    };
                }

                // Tạo giao dịch với số tiền đã trừ đi tiền đặt cọc
                await Transaction.create({
                    ...
                    payment_info: depositInfo ? { deposit_deducted: depositInfo } : null,
                    created_at: now,
                });
            }
...
const placeBid = async (bidData) => {
    try {
        ...
        // Kiểm tra nếu có đăng ký thì phải được approved và đã thanh toán đặt cọc
        if (registration) {
            if (registration.status !== "approved") {
                throw new Error("Đăng ký tham gia đấu giá của bạn chưa được duyệt");
            }
            if (registration.deposit_status !== "paid") {
                throw new Error("Bạn cần thanh toán tiền đặt cọc trước khi đặt giá");
            }
        }
...
const registerForAuction = async (auctionId, userId, depositAmount) => {
    try {
        ...
        // Lấy thông tin sản phẩm để tính toán số tiền đặt cọc
        ...
        const calculatedDepositAmount = parseFloat(product.starting_price) * 0.1;
        ...
        // Tạo transaction cho tiền đặt cọc
        ...
        return {
            ...
            data: {
                ...
                transaction: {
                    ...
                    transaction_code: depositTransaction.transaction_code,
